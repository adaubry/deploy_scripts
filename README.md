...
└────────┬────────┘
         ...
         ▼
┌──────────────────────────┐
│ deploy-pm2.service       │  (Deploy script runs)
│  - git pull              │
│  - docker compose up     │
│  - pm2 reload            │
└────────┬─────────────────┘
...
### 1. Systemd Service (`deploy-pm2.service`)

**Location:** `/etc/systemd/system/deploy-pm2.service`
...
After=network.target deploy-pm2.service
...
- `After=deploy-pm2.service` - Waits for deploy service to complete first
...
3. **Deploy Service Runs** (`deploy-pm2.service`)
...
sudo nano /etc/systemd/system/deploy-pm2.service
...
# Add "deploy-pm2.service" to the After= line

# 7. Enable services
sudo systemctl daemon-reload
sudo systemctl enable deploy-pm2.service
...
sudo systemctl status deploy-pm2.service
...
# View detailed logs
sudo journalctl -u deploy-pm2.service -n 100
...
sudo systemctl edit deploy-pm2.service --full
...
sudo systemctl start deploy-pm2.service    # Start service
sudo systemctl stop deploy-pm2.service     # Stop service
sudo systemctl restart deploy-pm2.service  # Restart service
sudo systemctl status deploy-pm2.service   # Check status
sudo systemctl enable deploy-pm2.service   # Enable on boot
sudo systemctl disable deploy-pm2.service  # Disable on boot

# View logs
sudo journalctl -u deploy-pm2.service      # View service logs
sudo journalctl -u deploy-pm2.service -f   # Follow logs live
...
3. **Check logs** - Always check both systemd logs and your deploy logs:
   ```bash
   sudo journalctl -u deploy-pm2.service -n 50
